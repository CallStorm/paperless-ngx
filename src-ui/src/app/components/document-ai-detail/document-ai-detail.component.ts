...
@Component({
  selector: 'pngx-document-ai-detail',
  templateUrl: './document-ai-detail.component.html',
  styleUrls: ['./document-ai-detail.component.scss'],
  imports: [
    PageHeaderComponent,
...
export class DocumentAiDetailComponent
  ...
  implements OnInit, OnDestroy, DirtyComponent
{
  ...
  DocumentAiDetailNavIDs = DocumentAiDetailNavIDs
  activeNavID: number
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap
      ...
      .subscribe((paramMap) => {
        ...
        if (section) {
          const navIDKey: string = Object.keys(DocumentAiDetailNavIDs).find(
            (navID) => navID.toLowerCase() == section
          )
          if (navIDKey) {
            this.activeNavID = DocumentAiDetailNavIDs[navIDKey]
          }
        } else if (paramMap.get('id')) {
          this.router.navigate(
            ['documents', +paramMap.get('id'), 'chat', 'content'],
            { replaceUrl: true }
          )
        }
      })
  ...
  onNavChange(navChangeEvent: NgbNavChangeEvent) {
    const [foundNavIDkey] = Object.entries(DocumentAiDetailNavIDs).find(
      ...
      .subscribe({
        next: ({ updateResult, nextDocId, closeResult }) => {
          ...
          if (closeResult && updateResult && nextDocId) {
            this.router.navigate(['documents', nextDocId, 'chat', 'details'])
  ...
  previousDoc() {
    this.documentListViewService
      ...
      .subscribe((prevDocId: number) => {
        this.router.navigate(['documents', prevDocId, 'chat', 'details'])
      })
  }
